...
class equation_popup:
    ...
    def __init__(self, wb):
        ...
        Entry(self.top_frame, textvariable=self.equation.tk_value, width=15, font=('Arial', 32)).pack(side=LEFT, fill=BOTH, expand=True)
        
        Button(self.top_frame, text="Done", font=('Arial', 20), command=self.quit).pack(side=RIGHT)
        
        Label(self.middle_frame, text='= ', font=('Arial', 32)).pack(side=LEFT)
        Label(self.middle_frame, textvariable=self.equation.answer, font=('Arial', 32)).pack(side=LEFT, fill=BOTH, expand=True)
    ...
    def quit(self):
        self.wb.canvas.bind('<ButtonPress-1>', self.wb.reset) #binds the button press back to reset, the original bind
        self.top.destroy()
    ...
    def graph(self):
        ...
        for P in self.equation.points:
            try:
                x, y = P[0]+width/2, (-1*P[1])+width/2 #translate math coords into canvas coords
                #trims the floats so they dont break tkinter with unreal numbers
                x, y = float("{:.10f}".format(x.real)), float("{:.10f}".format(y.real)) 

                if last_x and last_y:
                    self.canvas.create_line(last_x*gap, last_y*gap,
                                            x*gap, y*gap, fill="Red", width=self.canvas_width/self.width//5, tag="graph",
    ...
    def draw_lines(self):
        ...
        #x and y intercepts
        self.canvas.create_line((width//2)*gap, start, (width//2)*gap, end, tag="grid", width=3)
        self.canvas.create_line(start, (width//2)*gap, end, (width//2)*gap, tag="grid", width=3)

        for x in range(width):
            self.canvas.create_window(x*gap, (width//2)*gap, )
