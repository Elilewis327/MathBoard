...
class Select_Color(SubWindow):
    ...
    def draw(self):
        ...
        self.R = Scale(slider_frame, from_=0, to=255, orient=HORIZONTAL, length=500, showvalue=True)
        ...
        self.G = Scale(slider_frame, from_=0, to=255, orient=HORIZONTAL, length=500, showvalue=True)
        ...
        self.B = Scale(slider_frame, from_=0, to=255, orient=HORIZONTAL, length=500, showvalue=True)
        ...
        self.show_current_color = Button(upper_frame,
                                        height=5, width=10,
                                        bd='4', fg='white',
    ...
    def get_vals(self):
        ...
        """
        self.new_color  = '#%02x%02x%02x' % (self.R.get(), self.G.get(), self.B.get()) #this line is from https://stackoverflow.com/a/3380739
    ...
    def hex_to_rgb(self):
        ...
        hlen = len(hex_)
        return tuple(int(hex_[i:i+hlen//3], 16) for i in range(0, hlen, hlen//3))
...
class Select_Width(SubWindow):
    ...
    def __init__(self, current_width):
        ...
        self.width_slider = Scale(self.top, from_=0, to=50, orient=HORIZONTAL, length=500, showvalue=True)
